...
public class OrderRepository : IOrderRepository
{
    ...
    public OrderRepository(DapperContext context, IGridDataHelperRepository gridDataHelperRepository, IProductRepository productRepository, IShippingAddressRepository shippingAddressRepository)
    {
        _context = context;
    ...
    public async Task<Order> GetOrderById(int OrderId)
    {
        ...
        using (var con = _context.CreateConnection)
        {
            ...
            objOrder = await con.QueryFirstAsync<Order>("p_GET_Orders", param, commandType: CommandType.StoredProcedure);
        }
        return objOrder;
    }
    ...
    public async Task<Order> CreateOrder(OrderDTO objOrder)
    {
        ...
        float totalAmount = objOrder.CartItems.Sum(item => item.Product.Price * item.Quantity);  // Assume Product has a Price property
        float discountAmount = 100; // Assume fixed discount
        float grossAmount = totalAmount - discountAmount;
        float shippingAmount = 50; // Assume fixed shipping
        float netAmount = grossAmount + shippingAmount;
